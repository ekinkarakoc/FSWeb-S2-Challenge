...
function cumlelereDonustur(dizi, ayraç = ",") {
  const newArr = dizi.map((e) => {
    return e.join(ayraç);
  });
  return newArr;
}
...
function paragrafOlustur(dizi, cumleKur, cumlelereDonustur) {
  const newArr = cumlelereDonustur(dizi, " ");
...
console.log(emojileriDonustur("hem :D hem :d hem :o hem :O", emojiler));
